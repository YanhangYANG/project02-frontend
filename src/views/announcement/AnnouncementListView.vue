...
import AnnouncetmentsCard from "@/components/AnnouncetmentsCard.vue";
import type {Announcement, Student} from "@/type";
...
const announcements: Ref<Array<Announcement>> = ref([]);
...
AnnouncementService.getAnnouncements(3, props.page)
    .then((response: AxiosResponse<Announcement[]>) => {
      announcements.value = response.data;
      totalAnnouncement.value = response.headers["x-total-count"];
      loading.value = false;
    })
    .catch(() => {
...
onBeforeRouteUpdate((to, from, next) => {
  ...
  AnnouncementService.getAnnouncements(3, toPage)
      .then((response: AxiosResponse<Announcement[]>) => {
        announcements.value = response.data;
        totalAnnouncement.value = response.headers["x-total-count"];
        loading.value = false;
...
const hasNextPage = computed(() => {
  const totalPages = Math.ceil(totalAnnouncement.value / pageSize.value);
...
<template>
  ...
  <main class="flex flex-col items-center">
    <div class="container mt-12 grid grid-cols-3 place-items-center gap-12">
      <AnnouncetmentsCard v-for="announcement in announcements" :key="announcement.id" :announcement="announcement"></AnnouncetmentsCard>
    </div>
    <div class="flex flex-1"></div>
    <div class="pagination flex justify-between items-center mt-8 w-80">
      <div>
        <RouterLink :to="{ name: 'Announcements', query: { page: page - 1 } }" rel="prev" v-if="page != 1"
                    class="px-4 py-2 bg-[rgb(151,208,21)] hover:bg-[rgb(29,161,242)] rounded-md text-white outline-none focus:ring-4 shadow-lg transform active:scale-x-75 transition-transform mx-50 flex">
      ...
      <div>
        <RouterLink :to="{ name: 'Announcements', query: { page: page + 1 } }" rel="next" v-if="hasNextPage"
                    class="px-4 py-2 bg-[rgb(151,208,21)] hover:bg-[rgb(29,161,242)] rounded-md text-white outline-none focus:ring-4 shadow-lg transform active:scale-x-75 transition-transform mx-50 flex">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
               class="w-6 h-6">
